...
import com.android.fisewatchlauncher.utils.GifOpenHelper;
...
public class GifTextView extends AppCompatTextView {
    ...
    /**
     * @author Dragon SpanInfo 用于存储一个要显示的图片（动态或静态）的信息，
     *         包括分解后的每一帧mapList、替代文字的起始位置、终止位置
    ...
    /**
     * @param inputStr 要显示在textView上的文本
     */
    private boolean parseText(String inputStr) {
        ...
        while (mMatcher.find()) {
            ...
            if ((faceId = EmotionUtils.EMPTY_GIF_MAP.get(faceName)) != null) {
                if (isGif) {
                    parseGif(faceId, mMatcher.start(), mMatcher.end());//Gif图片进行解析
                } else {
                    parseBmp(faceId, mMatcher.start(), mMatcher.end());//解析静态图片
                }
            }
            hasGif = true;
        }
    ...
    public boolean parseMessage(GifTextView gifTextView) {
        if (gifTextView.myText != null && !gifTextView.myText.equals("")) {
            ...
            for (int i = 0; i < gifTextView.spanInfoList.size(); i++) { // for循环，处理显示多个图片的问题
                 ...
                 */
                int size = DpUtil.dp2px(gifTextView.getContext(), 30);
                if (gifCount != 0) {
