...
public class ChatFunctionFragment extends BaseFragment {
        ...
        switch (req) {
            case CROP_PHOTO:
                if (res == Activity.RESULT_OK) {
                    try {
                        sendImgMsg(imageUri.getPath());
                    } catch (Exception e) {
                        LogUtils.e("CROP_PHOTO error " + e);
                    }
                } else {
                    Log.d(MsgConstants.TAG, "失败");
                }
            ...
            case REQUEST_CODE_PICK_IMAGE:
                if (res == Activity.RESULT_OK) {
                    try {
                        ...
                        sendImgMsg(getRealPathFromURI(uri));
                    } catch (Exception e) {
                        e.printStackTrace();
                        Log.d(MsgConstants.TAG, e.getMessage());
                    }
                } else {
                    Log.d(MsgConstants.TAG, "失败");
                }
    ...
    private void sendImgMsg(String path) {
        WetMessage messageChatInfo = new WetMessage();
        ...
        messageChatInfo.setMessageContentType(WetMessage.MessageContentType.IMG);
        messageChatInfo.setTimestamp(System.currentTimeMillis());
        messageChatInfo.setImgPath(imageUri.getPath());
