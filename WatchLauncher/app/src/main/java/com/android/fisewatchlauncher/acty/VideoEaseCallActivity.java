...
public class VideoEaseCallActivity extends EaseCallActivity implements OnClickListener {
    ...
    class BrightnessDataProcess implements EMCameraDataProcessor {
        ...
        synchronized void setYDelta(byte yDelta) {
            Log.d("VideoEaseCallActivity", "brigntness uDelta:" + yDelta);
            this.yDelta = yDelta;
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EaseMobManager.instance().isVideoCalling = true;
        callType = 1;
...
//                                        callStateTextView.setText(s6);
                                ...
                                Toast.makeText(VideoEaseCallActivity.this, callStateTextView.getText(), Toast.LENGTH_SHORT).show();
                                postDelayedCloseMsg();
                            }
...
//                callHelper.startVideoRecord(PathUtil.getInstance().getVideoPath().getAbsolutePath());
        ...
        */
            ...
            case R.id.btn_capture_image:
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(VideoEaseCallActivity.this, "saved image to:" + filename, Toast.LENGTH_SHORT).show();
                    }
                });
                break;
            default:
    ...
    protected void onDestroy() {
        EaseMobManager.instance().isVideoCalling = false;
        stopMonitor();
        if (isRecording) {
            callHelper.stopVideoRecord();
            isRecording = false;
