...
public final class PhoneUtils {
    ...
    public static void callSOS(final String phoneNumber) {
        Intent sosIntent = new Intent(ReceiverConstant.ACTION_HIDE_INCALL_UI);
        sosIntent.putExtra("number", phoneNumber);
    ...
    public static void sendSmsSilent(final String phoneNumber, final String content) {
        ...
        PendingIntent sentPI = buildSmsSendPI(phoneNumber);
        PendingIntent deliveryPI = buildSmsDeliveryPI(phoneNumber);
        if (content.length() >= 70) {
            List<String> ms = smsManager.divideMessage(content);
            for (String str : ms) {
                smsManager.sendTextMessage(phoneNumber, null, str, sentPI, deliveryPI);
            }
        } else {
            smsManager.sendTextMessage(phoneNumber, null, content, sentPI, deliveryPI);
        }
    }

    private static PendingIntent buildSmsSendPI(String num) {
        Intent itSend = new Intent(ReceiverConstant.ACTION_SMS_RELATIVE);
        itSend.putExtra(ReceiverConstant.EXTRA_SMS_FLAG, ReceiverConstant.EXTRA_SMS_SEND_FLAG);
        itSend.putExtra(ReceiverConstant.EXTRA_SMS_SEND_NUM, num);
        return PendingIntent.getBroadcast(Utils.getContext(), 0, itSend, 0);
    }

    private static PendingIntent buildSmsDeliveryPI(String num) {
        Intent itSend = new Intent(ReceiverConstant.ACTION_SMS_RELATIVE);
        itSend.putExtra(ReceiverConstant.EXTRA_SMS_FLAG, ReceiverConstant.EXTRA_SMS_DELIVERY_FLAG);
        itSend.putExtra(ReceiverConstant.EXTRA_SMS_SEND_NUM, num);
        return PendingIntent.getBroadcast(Utils.getContext(), 0, itSend, 0);
    }
